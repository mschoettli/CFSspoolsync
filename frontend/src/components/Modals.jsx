...
import {
  X, Plus, Trash2, Wifi, Scale, Check, ArrowRight,
} from 'lucide-react'
...
export function AddSpoolModal({
  t, tares, editing, targetSlot, cfsGross, cfsConnected,
  onClose, onSave, onOpenTares,
}) {
  const [manufacturer, setManufacturer] = useState(editing?.manufacturer || '')
  const [material, setMaterial] = useState(editing?.material || 'PLA')
  const [color, setColor] = useState(editing?.color || '')
  const [colorHex, setColorHex] = useState(editing?.color_hex || '#22c55e')
  const [diameter, setDiameter] = useState(editing?.diameter || 1.75)
  const [nozzleTemp, setNozzleTemp] = useState(editing?.nozzle_temp || MATERIAL_PRESETS.PLA.nozzle)
  const [bedTemp, setBedTemp] = useState(editing?.bed_temp || MATERIAL_PRESETS.PLA.bed)
  const [grossWeight, setGrossWeight] = useState(
    editing?.gross_weight ?? (targetSlot && cfsGross ? Math.round(cfsGross) : 1200),
  )
  const [name, setName] = useState(editing?.name || '')

  // Material preset wählt automatisch Temperaturen
  useEffect(() => {
    if (editing) return
  ...
  return (
    ...
    >
      <div className="p-5 space-y-5">
        {targetSlot && cfsConnected && !editing && (
          <div className="flex items-start gap-3 p-3 rounded-lg border border-cyan-900/60 bg-cyan-950/30">
            ...
            <div className="flex-1 text-xs">
              ...
              <div className="mt-2 flex gap-4 font-mono">
                <span className="text-zinc-500">{t.detected}:</span>
                <span className="text-cyan-300">
                  {t.grossWeight}{' '}
                  <strong className="text-cyan-200">{fmt(cfsGross, 0)}g</strong>
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label={t.color} required>
            <div className="flex gap-2">
              <input
                ...
                placeholder="Jade White, Hyper Red..."
              />
              <input
                type="color"
                value={colorHex}
                onChange={(e) => setColorHex(e.target.value)}
                className="w-11 rounded-md border border-zinc-700 bg-zinc-800 cursor-pointer"
              />
            </div>
          </Field>
          ...
          <Field
            ...
            required
            hint={targetSlot && cfsConnected ? t.autoFilled : null}
          >
            <input
              type="number"
              value={grossWeight}
              ...
              className={`input ${
                targetSlot && cfsConnected ? 'border-cyan-800/60 bg-cyan-950/20' : ''
              }`}
            />
          </Field>
        </div>
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-950/50 p-3">
          ...
          <div className="mt-3 grid grid-cols-3 gap-3 text-center">
            ...
            <div>
              ...
              <div className="mt-0.5 text-lg font-semibold tabular-nums text-emerald-300">
                {fmt(netWeight, 0)} g
              </div>
            </div>
          </div>
...
export function TareTableModal({ t, tares, onCreate, onUpdate, onDelete, onClose }) {
  return (
    <Modal
      title={t.tareTableTitle}
      subtitle={t.tareTableSub}
      onClose={onClose}
      maxWidth="max-w-3xl"
    >
      <div className="p-5">
        <div className="flex justify-end mb-3">
          <button
            onClick={() =>
              onCreate({ manufacturer: 'Neu', material: 'PLA', weight: 200 })
            }
...
function TareRow({ tare, onSave, onDelete }) {
  ...
  // Debounced save bei Änderung (Lost-Focus oder Enter)
  const commit = () => {
    if (
      mfr !== tare.manufacturer ||
      mat !== tare.material ||
      +wt !== +tare.weight
    ) {
...
export function AssignSpoolModal({ t, slotId, shelfSpools, onClose, onAssign, onCreateNew }) {
  ...
  return (
    <Modal
      title={`${t.assignSpool} → ${t.slot} ${slotId}`}
      subtitle={t.selectSpool}
      onClose={onClose}
      maxWidth="max-w-lg"
    >
